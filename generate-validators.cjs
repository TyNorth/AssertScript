...
function generateValidatorsString(schemas) {
  ...
  for (const typeName in schemas) {
    ...
    // The generated function template starts here
    let functionBody = `export function validate${typeName}(data, options = {}) {\n`
    ...
    for (let key in schema) {
      ...
      const errorHandlingLogic = (rule, message) => {
        ...
        const errorObject = `{ property: '${propertyName}', rule: '${rule}', message: \`${message.replace(/`/g, '\\`')}\`, value: data.${propertyName} }`
        return `if (isVerbose) { errors.push(${errorObject}); } else { throw new Error(\`${errorMessage.replace(/`/g, '\\`')}\`); }`
      }

      let validationCode = `  if (data.hasOwnProperty('${propertyName}')) {\n`

      if (schemas[type]) {
        validationCode += `    try { validate${type}(data.${propertyName}, options); } catch (e) { if (!isVerbose) { throw new Error(\`Validation failed for ${typeName}.${propertyName}: \` + e.message); } else { errors.push({ property: '${propertyName}', rule: 'nested', message: e.message }); } }\n`
      } else {
        const typeCheck =
          type === 'array'
            ? `!Array.isArray(data.${propertyName})`
            : `typeof data.${propertyName} !== '${type}'`
        validationCode += `    if (${typeCheck}) { ${errorHandlingLogic('type', `expected type '${type}' but got '\${typeof data.${propertyName}}'`)} }\n`

        if (type === 'string' && rules.minLength)
          validationCode += `    if (data.${propertyName}.length < ${rules.minLength}) { ${errorHandlingLogic('minLength', `must be at least ${rules.minLength} characters long.`)} }\n`
        if (type === 'number' && rules.min !== undefined)
          validationCode += `    if (data.${propertyName} < ${rules.min}) { ${errorHandlingLogic('min', `must be at least ${rules.min}.`)} }\n`
      }
      validationCode += `  } else if (!${isOptional}) {\n`
      validationCode += `    ${errorHandlingLogic('required', 'is a required property.')}\n`
      validationCode += `  }\n\n`
      functionBody += validationCode
    }

    functionBody += `  if (isVerbose) return { isValid: errors.length === 0, errors, value: data };\n`
