...
function generateValidatorsString(schemas) {
  let outputCode = `// THIS FILE IS AUTO-GENERATED BY ASSERTSCRIPT. DO NOT EDIT.\n`
      ...
      if (schemas[finalType]) {
        ...
        if (rules.type === 'string') {
          ...
          if (rules.maxLength !== undefined)
            functionBody += `      if (String(sanitizedValue.${propertyName}).length > ${rules.maxLength}) { ${errorHandlingLogic('maxLength', `must be no more than ${rules.maxLength} characters long.`)} }\n`
          if (rules.pattern)
            functionBody += `      if (!/${rules.pattern}/.test(sanitizedValue.${propertyName})) { ${errorHandlingLogic('pattern', `does not match required format.`)} }\n`
          if (rules.enum)
            functionBody += `      if (!${JSON.stringify(rules.enum)}.includes(sanitizedValue.${propertyName})) { ${errorHandlingLogic('enum', `must be one of [${rules.enum.join(', ')}].`)} }\n`
          if (rules.startsWith)
        ...
        if (finalType === 'number') {
          ...
          if (rules.max !== undefined)
            functionBody += `      if (sanitizedValue.${propertyName} > ${rules.max}) { ${errorHandlingLogic('max', `must be no more than ${rules.max}.`)} }\n`
          if (rules.isInteger)
        ...
        if (rules.type === 'array') {
          ...
          if (rules.maxLength !== undefined)
            functionBody += `      if (sanitizedValue.${propertyName}.length > ${rules.maxLength}) { ${errorHandlingLogic('maxLength', `must contain no more than ${rules.maxLength} items.`)} }\n`
          if (rules.items) {
