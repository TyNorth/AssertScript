...
// This imports the real validator generated by your tool!
// Make sure to run `npm run generate` or have the watcher running after updating types.json
...
const formData = reactive({
  id: null, // Initialize as null for numbers to align with validation
  username: '',
  email: '',
  age: null, // Initialize as null for numbers
  status: 'active', // Default value for the select dropdown
  userClass: '',
})

// Reactive errors object, mirroring formData structure
...
const showSuccessMessage = ref(false) // Renamed for clarity

// --- COMPUTED PROPERTIES ---

// Use a computed property for form validity, derived from errors
const isFormValid = computed(() => {
  // Check if all error fields are empty
  return (
    Object.values(errors).every((error) => error === '') &&
    Object.keys(sanitizedData.value).length > 0 && // Ensure sanitized data exists
    !sanitizedData.value.error
  ) // And that it doesn't contain a generic error
})
...
watch(
  formData,
  (newData) => {
    showSuccessMessage.value = false // Hide success message on any form change

    // Prepare data for validation: convert empty strings to null for number fields.
    const dataToValidate = {
      ...newData,
      id: newData.id === '' ? null : Number(newData.id),
      age: newData.age === '' ? null : Number(newData.age),
    }

    // Remove optional empty fields before validation if the validator prefers
    // them to be absent rather than null, depending on your `types.json` schema.
    if (dataToValidate.age === null) {
      delete dataToValidate.age
    }

    const result = validateUser(dataToValidate, { verbose: true })

    // Clear all existing errors
    Object.keys(errors).forEach((key) => (errors[key] = ''))

    if (result.isValid) {
      sanitizedData.value = result.value
    } else {
      sanitizedData.value = { error: 'Form data is currently invalid.' }
      // Populate errors based on the validation result
      for (const error of result.errors) {
        if (errors.hasOwnProperty(error.property)) {
          // Only assign the first error message per field
...
const handleRegister = () => {
  // The submit button is disabled if form is invalid, so this check is a safeguard
  ...
  // Also clear errors immediately after submission
  Object.keys(errors).forEach((key) => (errors[key] = ''))
}
</script>
